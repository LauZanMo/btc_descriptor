...
typedef struct Plane {
    ...
    void sync_p_center() {
        p_center_.x        = center_(0);
        p_center_.y        = center_(1);
        p_center_.z        = center_(2);
        p_center_.normal_x = normal_(0);
        p_center_.normal_y = normal_(1);
        p_center_.normal_z = normal_(2);
    }

    void update_from_covariance(float detection_thre) {
        Eigen::EigenSolver<Eigen::Matrix3d> es(covariance_);
        ...
        if (evalsReal(evalsMin) < detection_thre) {
            normal_ << evecs.real()(0, evalsMin), evecs.real()(1, evalsMin), evecs.real()(2, evalsMin);
            min_eigen_value_ = evalsReal(evalsMin);
            radius_          = sqrt(evalsReal(evalsMax));
            is_plane_        = true;
            d_               = -normal_.dot(center_);
            sync_p_center();
        } else {
            is_plane_ = false;
        }
...
public:
    ...
    BTC_LOC(int64_t vx = 0, int64_t vy = 0, int64_t vz = 0, int64_t va = 0, int64_t vb = 0, int64_t vc = 0)
...
struct PlaneSolver {
    ...
    static ceres::CostFunction *Create(const Eigen::Vector3d curr_point_, const Eigen::Vector3d curr_normal_,
...
public:
    ...
    void GenerateBtcDescs(const pcl::PointCloud<pcl::PointXYZI>::Ptr &input_cloud, const int frame_id,
                          std::vector<BTC> &btcs_vec);

    // search result <candidate_id, plane icp score>. -1 for no loop
    void SearchLoop(const std::vector<BTC> &btcs_vec, std::pair<int, double> &loop_result,
    ...
    void triangle_solver(std::pair<BTC, BTC> &std_pair, Eigen::Vector3d &t, Eigen::Matrix3d &rot);
...
private:
    ...
    void assign_merge_ids(std::vector<std::shared_ptr<Plane>> &planes);

    void merge_two_planes(std::shared_ptr<Plane> &target, const std::shared_ptr<Plane> &source);

    // extract corner points from pre-build voxel map and clouds

    void binary_extractor(const std::vector<std::shared_ptr<Plane>> proj_plane_list,
                          const pcl::PointCloud<pcl::PointXYZI>::Ptr &input_cloud,
                          std::vector<BinaryDescriptor> &binary_descriptor_list);

    void extract_binary(const Eigen::Vector3d &project_center, const Eigen::Vector3d &project_normal,
                        const pcl::PointCloud<pcl::PointXYZI>::Ptr &input_cloud,
    ...
    void generate_btc(const std::vector<BinaryDescriptor> &binary_list, const int &frame_id,
                      std::vector<BTC> &btc_list);

    // Select a specified number of candidate frames according to the number of
    // BtcDescs rough matches
    void candidate_selector(const std::vector<BTC> &btcs_vec, std::vector<BTCMatchList> &candidate_matcher_vec);

    // Get the best candidate frame by geometry check
    void candidate_verify(const BTCMatchList &candidate_matcher, double &verify_score,
                          std::pair<Eigen::Vector3d, Eigen::Matrix3d> &relative_pose,
                          std::vector<std::pair<BTC, BTC>> &sucess_match_vec);

    int count_triangle_inliers(const std::vector<std::pair<BTC, BTC>> &matches, const Eigen::Matrix3d &rot,
                               const Eigen::Vector3d &t, double threshold);

    void collect_triangle_inliers(const std::vector<std::pair<BTC, BTC>> &matches, const Eigen::Matrix3d &rot,
                                  const Eigen::Vector3d &t, double threshold,
