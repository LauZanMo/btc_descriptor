        ...
        while (getline(sin, info, ',')) {
            if (number == 0) {
                ...
                if (number == 7) {
                    ...
                    Eigen::Quaterniond q(temp_matrix[3], temp_matrix[4], temp_matrix[5], temp_matrix[6]);
...
void load_cu_pose_with_time(const std::string &pose_file,
                            std::vector<std::pair<Eigen::Vector3d, Eigen::Matrix3d>> &pose_list,
                            std::vector<double> &time_list) {
    time_list.clear();
    pose_list.clear();
    ...
    while (getline(fin, line)) {
        ...
        while (getline(sin, info, ',')) {
            if (number == 0) {
                ...
                temp_matrix[number - 1] = p;
                if (number == 12) {
                    Eigen::Vector3d translation(temp_matrix[3], temp_matrix[7], temp_matrix[11]);
                    Eigen::Matrix3d rotation;
                    rotation << temp_matrix[0], temp_matrix[1], temp_matrix[2], temp_matrix[4], temp_matrix[5],
                            temp_matrix[6], temp_matrix[8], temp_matrix[9], temp_matrix[10];
                    std::pair<Eigen::Vector3d, Eigen::Matrix3d> single_pose;
                    single_pose.first  = translation;
...
void load_pose_with_frame(const std::string &pose_file,
                          std::vector<std::pair<Eigen::Vector3d, Eigen::Matrix3d>> &pose_list,
...
void load_evo_pose_with_time(const std::string &pose_file,
                             std::vector<std::pair<Eigen::Vector3d, Eigen::Matrix3d>> &pose_list,
                             std::vector<double> &time_list) {
    time_list.clear();
    pose_list.clear();
    ...
    while (getline(fin, line)) {
        ...
        while (getline(sin, info, ' ')) {
            if (number == 0) {
                ...
                if (number == 7) {
                    Eigen::Vector3d translation(temp_matrix[0], temp_matrix[1], temp_matrix[2]);
                    Eigen::Quaterniond q(temp_matrix[6], temp_matrix[3], temp_matrix[4], temp_matrix[5]);
                    std::pair<Eigen::Vector3d, Eigen::Matrix3d> single_pose;
                    single_pose.first  = translation;
                    single_pose.second = q.toRotationMatrix();
                    pose_list.push_back(single_pose);
...
double calc_overlap(const pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud1,
                    const pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud2, double dis_threshold) {
    int point_kip    = 2;
    double match_num = 0;
...
double calc_overlap(const pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud1,
                    const pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud2, double dis_threshold, int skip_num) {
    double match_num = 0;
    pcl::KdTreeFLANN<pcl::PointXYZI>::Ptr kd_tree(new pcl::KdTreeFLANN<pcl::PointXYZI>);
    kd_tree->setInputCloud(cloud2);
    std::vector<int> pointIdxNKNSearch(1);
    ...
    //           match_num
    //           << std::endl;
    double overlap = (2 * match_num * skip_num) / (cloud1->size() + cloud2->size());
    return overlap;
}
